...
comp1·comp2·comp3에 걸친 멀티 컴포넌트 인프라에서 **노드 과잉 프로비저닝**, **태그 거버넌스 공백**, **ECR 이미지 누적** 세 갈래의 비효율이 확인된다.
핵심 이슈는 다음과 같다.
- **EKS 노드 과잉 프로비저닝 (high)**: comp1의 m5.2xlarge 노드 20대가 평균 CPU 15.0%만 쓰면서 플릿을 부풀리고 있다. pod request를 실제 사용량에 맞추고 초과 노드를 축소한다.
- **비용 할당 태그 누락 (medium)**: EKS·EC2·ECR·RDS·S3·LB 등 76개 리소스에 cost-center/team/environment/project 태그가 0.0% 덮여 있다. 전체 지출의 49.6%가 미할당으로 잡힌다.
- **ECR lifecycle 정책 부재 (medium)**: comp2의 ECR repo 3곳이 정책 없이 평균 370개의 이미지를 쌓고 있다.
권장 순서는 이렇다.
1. comp1 EKS 노드 우선 축소 — pod request 재산정 후 m5.2xlarge 플릿을 사용률 기준으로 줄인다.
2. provider `default_tags`와 CostCenter 필수 태그 정책 도입 — 76개 리소스의 비용 가시성을 회복한다.
3. comp2 ECR 3개 repo에 lifecycle 정책 추가 — 오래된 이미지를 자동 정리한다.
...
| --- | ---: | ---: | ---: | ---: |
| Single | - | - | 0 | 0 | -s |
| Multi | 1.0 | 74 | 841 | 20066 | 59.659107s |
